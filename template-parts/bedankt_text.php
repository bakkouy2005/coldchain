                ...
                ?>
                    <div class="bg-white shadow-lg rounded-xl border border-gray-200 overflow-hidden transition-shadow duration-300 hover:shadow-xl">
                        ...
                        <button type="button"
                            ...
                            <i class="fa-solid fa-plus text-gray-500 transition-transform duration-700 ease-in-out transform toggle-icon"></i>
                        </button>

                        <!-- Content -->
                        <?php if ($q_text_area) : ?>
                            <div class="max-h-0 opacity-0 overflow-hidden transition-all duration-700 ease-in-out text-area-<?php echo $index; ?> px-4 pb-4">
                                <p class="text-gray-700 text-sm mt-2 transition-opacity duration-700 ease-in-out opacity-0"><?php echo esc_html($q_text_area); ?></p>
                            </div>
                        <?php endif; ?>
                    </div>
                <?php endforeach; ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    toggles.forEach(toggle => {
        toggle.addEventListener('click', () => {
            ...
            if (textArea.classList.contains('max-h-0')) {
                // Open: slide + fade
                textArea.classList.remove('max-h-0', 'opacity-0');
                textArea.classList.add('max-h-screen', 'opacity-100');
                ...
                // Close: slide + fade
                textArea.classList.add('max-h-0', 'opacity-0');
                textArea.classList.remove('max-h-screen', 'opacity-100');
