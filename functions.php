...
add_action('af/form/submission', function( $form, $fields, $args ) {
    ...
    <table width="100%" cellpadding="10" cellspacing="0" style="background:#fff; border-radius:8px;">
        ...
        <?php if ($vacature_functie): ?><tr><td><strong>Vacature functie:</strong></td><td><?= esc_html($vacature_functie) ?></td></tr><?php endif; ?>
        <?php if ($naam): ?><tr><td><strong>Naam:</strong></td><td><?= esc_html($naam) ?></td></tr><?php endif; ?>
        <?php if ($email): ?><tr><td><strong>E-mail:</strong></td><td><?= esc_html($email) ?></td></tr><?php endif; ?>
        <?php if ($woonplaats): ?><tr><td><strong>Woonplaats:</strong></td><td><?= esc_html($woonplaats) ?></td></tr><?php endif; ?>
        <?php if ($telefoonnummer): ?><tr><td><strong>Telefoonnummer:</strong></td><td><?= esc_html($telefoonnummer) ?></td></tr><?php endif; ?>
        <?php if ($contactvoorkeur): ?><tr><td><strong>Contactvoorkeur:</strong></td><td><?= esc_html($contactvoorkeur) ?></td></tr><?php endif; ?>
        <?php if ($cv_document): ?><tr><td><strong>CV Document:</strong></td><td><?= esc_html($cv_document) ?></td></tr><?php endif; ?>
        <?php if ($bericht): ?><tr><td><strong>Bericht:</strong></td><td><?= nl2br(esc_html($bericht)) ?></td></tr><?php endif; ?>
    </table>
    </body>
    </html>
    <?php
    ...
    if ($email) {
        ...
        $confirm_message = '<html><body style="font-family: Arial, sans-serif; background-color:#0a131f; color:#fff; padding:40px; text-align:center;">'
            . '<img src="' . esc_url($logo_url) . '" alt="Coldchain Logo" width="150" style="margin-bottom:20px;">'
            . '<h2 style="color:#fff;">Bedankt voor uw sollicitatie, ' . esc_html($naam ?: 'kandidaat') . '!</h2>'
            . '<p style="color:#ccc;">Wij hebben uw sollicitatie ontvangen en nemen spoedig contact met u op.</p>'
            . '<p style="color:#808080; margin-top:20px;">Met vriendelijke groet,<br>Het team van Cold-Chain Logistik Services</p>'
            . '</body></html>';
